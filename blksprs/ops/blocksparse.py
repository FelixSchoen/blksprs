...
class BlocksparseRowWiseMax(BaseBlocksparse):
    # TODO No gradient at the moment
    ...
    def forward(self, x: Tensor, sparsity_layout: Tensor) -> Tensor:
        ...
        return _BlocksparseRowWiseMax.apply(x,
                                            sparsity_layout, sparsity_lut, sparsity_reverse_lut,
                                            self.sparsity_block_size, self.triton_block_size, self.device)


class _BlocksparseRowWiseMax(torch.autograd.Function):

    @staticmethod
    def forward(ctx, x: Tensor,
                sparsity_layout: Tensor, sparsity_lut: Tensor, sparsity_reverse_lut: Tensor,
                sparsity_block_size: int, triton_block_size: int, device: torch.device) -> Tensor:
        x_b, x_r, x_c = x.size()
        s_l_b, s_l_r, s_l_c = sparsity_layout.size()
        s_l_b_s, s_l_r_s, s_l_c_s = sparsity_layout.stride()
        s_lut_r, s_lut_c = sparsity_lut.size()
        ...
        output = torch.zeros(size=(s_l_b, s_l_r, 1), device=device)
        ...
        (_BlocksparseRowWiseMax.kernel_blocksparse_row_wise_max[triton_grid]
         (x,
          output, o_b, o_b_s, o_r, o_r_s, o_c, o_c_s,
          s_l_b, s_l_b_s, s_l_r, s_l_r_s, s_l_c, s_l_c_s,
    ...
    def kernel_blocksparse_row_wise_max(x,
        ...
        for i_seg_tri in range(0, tl.cdiv(s_l_c * sparsity_block_size, TRITON_BLOCK_SIZE)):
            ...
            if rev_idx_spa >= 0:
                ...
                buf = tl.maximum(buf, tl.reshape(tl.max(blk, axis=-1), (TRITON_BLOCK_SIZE, 1)))

                # DEBUG
                d_idx = (pid_blk * o_b_s +
                         ((pid_row * TRITON_BLOCK_SIZE + tl.arange(0, TRITON_BLOCK_SIZE)) * o_r_s)[:, None] +
                         ((i_seg_tri_mod * TRITON_BLOCK_SIZE + tl.arange(0, TRITON_BLOCK_SIZE)) * o_c_s)[None, :])
                d_msk = (d_idx < o_b * o_b_s + o_r * o_r_s + o_c * o_c_s)
                tl.store(debug_tensor + d_idx, blk, d_msk)

        o_idx = (spa_bat * o_b_s +
                 ((spa_row * sparsity_block_size + pid_row * TRITON_BLOCK_SIZE
                   + tl.arange(0, TRITON_BLOCK_SIZE)) * o_r_s)[:, None] +
                 ((tl.arange(0, 1)) * o_c_s)[None, :])
        o_msk = (o_idx < o_b * o_b_s + o_r * o_r_s + o_c * o_c_s)
