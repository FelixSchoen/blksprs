...
def matmul_kernel(x,
    ...
    for i_seg_tri in range(0, tl.cdiv(s_l_x_c * sparsity_block_size, TRITON_BLOCK_SIZE)):
        ...
        rev_idx_spa_y_msk = ((rev_idx_spa_y_idx >= 0) &
                             (rev_idx_spa_y_idx < tl.cast(s_l_y_b, tl.int64) * s_l_y_b_s))
        ...
        if rev_idx_spa_x >= 0 and rev_idx_spa_y >= 0:
            blk_x_idx = ((tl.cast(rev_idx_spa_x, tl.int64) * x_b_s) +
                         ...
                         ((i_seg_tri_mod * TRITON_BLOCK_SIZE + tl.cast(tl.arange(0, TRITON_BLOCK_SIZE), tl.int64)) * x_c_s)[None, :])
            blk_x_msk = ((blk_x_idx >= 0) &
                         (blk_x_idx < tl.cast(x_b, tl.int64) * x_b_s))
            blk_x = tl.load(x + blk_x_idx, mask=blk_x_msk)

            blk_y_idx = ((tl.cast(rev_idx_spa_y, tl.int64) * y_b_s) +
                         ((i_seg_tri_mod * TRITON_BLOCK_SIZE + tl.cast(tl.arange(0, TRITON_BLOCK_SIZE), tl.int64)) * y_r_s)[:, None] +
                         ((pid_col * TRITON_BLOCK_SIZE + tl.cast(tl.arange(0, TRITON_BLOCK_SIZE), tl.int64)) * y_c_s)[None, :])
            blk_y_msk = ((blk_y_idx >= 0) &
                         (blk_y_idx < tl.cast(y_b, tl.int64) * y_b_s))
