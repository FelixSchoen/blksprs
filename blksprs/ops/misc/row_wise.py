...
def row_wise_sum(x: BlksprsTensor, sparsity_layout: Tensor, sparsity_block_size: int,
                 flag_slice_only: Literal[False] = False) -> tuple[BlksprsTensor, Tensor]:
    ...


@overload
def row_wise_sum(x: BlksprsTensor, sparsity_layout: Tensor, sparsity_block_size: int,
                 flag_slice_only: Literal[True]) -> tuple[Tensor, Tensor]:
    ...


@overload
def row_wise_sum(x: BlksprsTensor, sparsity_layout: Tensor, sparsity_block_size: int,
                 flag_slice_only: bool) -> tuple[BlksprsTensor | Tensor, Tensor]:
    ...


@torch.amp.custom_fwd(device_type="cuda")
def row_wise_sum(x: BlksprsTensor, sparsity_layout: Tensor, sparsity_block_size: int,
                 flag_slice_only: bool = False) -> tuple[BlksprsTensor | Tensor, Tensor]:
    """Computes the row-wise sum of a block-sparse tensor.

    The output layout contains one active block per active block row. By default, the first column of each output block
    contains the row sums.

    Note:
        When ``flag_slice_only`` is set, the values are returned as a regular :class:`torch.Tensor` with shape
        ``(active_block_rows, sparsity_block_size, 1)``. This compact auxiliary representation is accepted by
        :func:`row_wise_add` and :func:`row_wise_sub`, but it is not a square-block :class:`BlksprsTensor` and cannot be
        passed to general block-sparse operations such as :func:`blksprs.ops.to_dense`.
    ...
    Args:
        ...
        flag_slice_only (bool, optional): Whether to return compact one-column slices instead of square blocks
            (default ``False``).

    Returns:
        tuple[BlksprsTensor | Tensor, Tensor]: The row sums and their sparsity layout. The values are a square-block
            :class:`BlksprsTensor` by default and a regular :class:`torch.Tensor` when ``flag_slice_only`` is set.
    ...
    sparsity_layout_output = as_base_tensor(
        torch.any(sparsity_layout, dim=-1, keepdim=True))
    ...
    if x.size(0) == 0:
        output = torch.empty(
            (0, sparsity_block_size, 1 if flag_slice_only else sparsity_block_size),
            dtype=x.dtype,
            device=x.device,
        )
        if flag_slice_only:
            return as_base_tensor(output), sparsity_layout_output
        return BlksprsTensor.wrap(output), sparsity_layout_output

    output = row_wise_sum_forward(
        x, layout_indices, sparsity_layout_output, packed_indices_output,
        sparsity_block_size, n_sparse_blocks_output, flag_slice_only)
    if flag_slice_only:
        return as_base_tensor(output), sparsity_layout_output
    return BlksprsTensor.wrap(output), sparsity_layout_output
...
def row_wise_sum_forward(x: Tensor, layout_indices: Tensor,
    ...
    with torch.no_grad():
        # Multiple input blocks contribute to each output row. Accumulating
        # those partial sums directly in float16 or bfloat16 can saturate long
        # before the mathematically correct result exceeds the dtype range.
...
def row_wise_sum_kernel(x,
    ...
    packed_idx = tl.cast(tl.load(pidx_o + packed_idx_idx, mask=packed_idx_msk, other=-1), index_dtype)
...
def row_wise_max(x: BlksprsTensor, sparsity_layout: Tensor, sparsity_block_size: int,
                 flag_slice_only: bool = False) -> tuple[BlksprsTensor | Tensor, Tensor]:
    """Computes the row-wise maximum of a block-sparse tensor.

    The output layout contains one active block per active block row. By default, the first column of each output block
    contains the row maxima.
    ...
    Returns:
        tuple[BlksprsTensor | Tensor, Tensor]: The row maxima and their sparsity layout. The values are a square-block
            :class:`BlksprsTensor` by default and a regular :class:`torch.Tensor` when ``flag_slice_only`` is set.
...
def row_wise_max_forward(x: Tensor, layout_indices: Tensor,
    ...
    with torch.no_grad():
        output = torch.full(size=(n_sparse_blocks_output,
                            ...
                            dtype=torch.float32,
                            device=x.device)
        nan_rows = torch.zeros(
            size=(n_sparse_blocks_output, sparsity_block_size, 1),
            dtype=torch.int32,
            device=x.device,
        )
        ...
        nan_b = nan_rows.size(0)
        nan_b_s, nan_r_s, _ = stride(nan_rows)
        s_l_o_b, s_l_o_r, s_l_o_c = sparsity_layout_output.size()
        s_l_o_b_s, s_l_o_r_s, s_l_o_c_s = stride(sparsity_layout_output)
        ...
        (wrap_triton(row_wise_max_kernel)[triton_grid]
         (x,
          ...
          o_b, o_b_s, o_r_s,
          nan_rows,
          nan_b, nan_b_s, nan_r_s,
          s_l_o_b, s_l_o_b_s, s_l_o_r_s,
          packed_indices_output,
          sparsity_block_size,
          USE_INT64=use_int64))

        output[..., :1].masked_fill_(nan_rows != 0, float("nan"))
...
def row_wise_max_kernel(x,
    ...
    if packed_idx >= 0:
        ...
        buf_nan = tl.cast(
            tl.reshape(tl.max(blk != blk, axis=-1), (TRITON_BLOCK_SIZE, 1)),
            tl.int32,
        )
        ...
        nan_idx = (tl.cast(packed_idx, index_dtype) * nan_b_s +
                   ((pid_row * TRITON_BLOCK_SIZE +
                     tl.cast(tl.arange(0, TRITON_BLOCK_SIZE), index_dtype)) * nan_r_s)[:, None])
        nan_msk = ((nan_idx >= 0) &
                   (nan_idx < tl.cast(nan_b, index_dtype) * nan_b_s))
        tl.atomic_or(nan_rows + nan_idx, buf_nan, nan_msk)


@torch.amp.custom_fwd(device_type="cuda")
def row_wise_add(x: BlksprsTensor, sparsity_layout_x: Tensor, y: Tensor,
                 sparsity_block_size: int) -> BlksprsTensor:
    """Adds one row-wise value to every element in the corresponding row of ``x``.
    ...
    Args:
        ...
        y (Tensor): A compressed row-wise tensor with one value per row. Both
            compact ``[..., 1]`` slices and square ``[..., sparsity_block_size]``
            blocks are accepted.
        sparsity_block_size (int): The size of the sparsity blocks.

    Returns:
        BlksprsTensor: The row-wise sums in compressed form.
    ...
    layout_indices_x = build_layout_indices(sparsity_layout_x)

    sparsity_layout_rwm = as_base_tensor(
        torch.any(sparsity_layout_x, dim=-1, keepdim=True))
...
def row_wise_sub(x: BlksprsTensor, sparsity_layout_x: Tensor, y: Tensor,
                 sparsity_block_size: int) -> BlksprsTensor:
    """Subtracts one row-wise value from every element in the corresponding row of ``x``.

    This is a convenience wrapper around :func:`row_wise_add` with ``y`` negated.

    Args:
        x (BlksprsTensor): The compressed input tensor.
        sparsity_layout_x (Tensor): The sparsity layout of ``x``.
        y (Tensor): A compressed row-wise tensor with one value per row.
        sparsity_block_size (int): The size of the sparsity blocks.

    Returns:
        BlksprsTensor: The row-wise differences in compressed form.
...
def row_wise_add_kernel(x,
    ...
    packed_idx_s = tl.cast(tl.load(pidx_y + packed_idx_s_idx, mask=packed_idx_s_msk, other=-1), index_dtype)
