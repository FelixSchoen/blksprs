...
def scatter(x: Tensor, sparsity_layout_x: Tensor,
            i: Tensor, sparsity_layout_output: Tensor,
    ...
    validate_contiguous(sparsity_layout_x, sparsity_lut_x, sparsity_reverse_lut_x,
                        sparsity_layout_output, sparsity_lut_o, sparsity_reverse_lut_o)

    return _BlocksparseScatter.apply(x, sparsity_layout_x, sparsity_lut_x, sparsity_reverse_lut_x,
                                     i,
                                     sparsity_layout_output, sparsity_lut_o, sparsity_reverse_lut_o,
                                     sparsity_block_size, n_sparse_blocks, triton_block_size)


class _BlocksparseScatter(torch.autograd.Function):

    @staticmethod
    def forward(ctx, x: Tensor, sparsity_layout_x: Tensor, sparsity_lut_x: Tensor, sparsity_reverse_lut_x: Tensor,
                i: Tensor,
                sparsity_layout_o: Tensor, sparsity_lut_o: Tensor, sparsity_reverse_lut_o: Tensor,
        ...
        (_BlocksparseScatter.kernel_blocksparse_scatter[triton_grid]
         (x,
          x_b, x_b_s, x_r, x_r_s, x_c, x_c_s,
          s_l_x_b, s_l_x_b_s, s_l_x_r, s_l_x_r_s, s_l_x_c, s_l_x_c_s,
          sparsity_lut_x, s_lut_x_r, s_lut_x_r_s, s_lut_x_c, s_lut_x_c_s,
          ...
          sparsity_lut_o, s_lut_o_r, s_lut_o_r_s, s_lut_o_c, s_lut_o_c_s,
    ...
    def kernel_blocksparse_scatter(x,
        ...
        rev_idx_spa_o = tl.load(r_lut_x + rev_idx_spa_o_idx, mask=rev_idx_spa_o_msk).to(tl.int32)
